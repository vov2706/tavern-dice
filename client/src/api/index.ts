...
const STORAGE_TOKEN_KEY = 'tavern.session.token'
const STORAGE_USER_KEY = 'tavern.session.user'
...
const makeRoomCode = () => {
  ...
  const b = '23456789'
  const pick = (s: string) => s[rand(0, s.length - 1)]
  return `TAV-${pick(a)}${pick(a)}${pick(b)}${pick(b)}`
}
...
export const gameApi: GameApi = {
  ...
  async login(input) {
    ...
    const session: Session = {
      ...
      user: { id: crypto.randomUUID(), username: input.username },
    }
  ...
  async register(input) {
    ...
    return this.login({ username: input.username, password: input.password })
  },
